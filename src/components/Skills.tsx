...
import { SiNextdotjs, SiTailwindcss, SiVite, SiGit, SiVitest } from "react-icons/si";
...
export default function Skills() {
  ...
  const skills: SkillItem[] = [
    ...
    { name: "Next.js", iconNode: <SiNextdotjs className="h-10 w-10 text-white" />, level: 85 },
    { name: "Tailwind CSS", iconNode: <SiTailwindcss className="h-10 w-10 text-cyan-400" />, level: 90 },
    { name: "Vite", iconNode: <SiVite className="h-10 w-10 text-purple-400" />, level: 80 },
    { name: "Git", iconNode: <SiGit className="h-10 w-10 text-orange-500" />, level: 85 },
    { name: "Vitest", iconNode: <SiVitest className="h-10 w-10 text-emerald-400" />, level: 75 },
  ];

  return (
    <section id="skills" className="scroll-mt-20 md:scroll-mt-24 rounded-br-[80px] bg-gradient-to-tr from-slate-950 to-slate-900 md:rounded-br-[180px]">
      <div className="container mx-auto max-w-4xl p-4 py-12">
        <div className="relative mb-4 p-4 text-center">
          <h2 className="relative mb-2 font-bold">
            <span className="mr-2 font-headline text-3xl text-white">Educação &</span>
            <span className="font-handwriting text-4xl text-white">Skills</span>
          </h2>
          <p className="relative text-sm text-white">Front-end Developer | React.js | TypeScript | JavaScript</p>
        ...
        <div className="flex flex-col gap-8 md:flex-row">
          <div className="basis-1/2">
            ...
            {educations.map((education, index) => (
              <div key={`education-${index}`} className="mb-2 rounded-lg bg-white/5 p-4 text-sm font-semibold text-white ring-1 ring-white/10">
                {education.name}
              </div>
            ))}
          </div>
